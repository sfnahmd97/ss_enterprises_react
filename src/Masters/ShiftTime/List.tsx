...
export default function Main() {
  ...
  const fetchDesignTypes = async (page = 1, search = "") => {
    try {
      ...
      const res = await api.get<ListApiResponse<any[]>>(`/shift-time`, {
        params,
      });
  ...
  const changeStatus = (id: number) => {
    confirmAlert(
      ...
      async () => {
        try {
          setStatusLoading(id);
          const res = await api.put(`/shift-time/change-status/${id}`);
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
              <div className="flex items-center">
                ...
                <span className="text-gray-700 font-medium">Shift Time</span>
              </div>
            </li>
          </ol>
        </nav>
                ...
                design_types.map((val, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center space-x-2">
                      <Link
                        to={`/master/shift-time/edit/${val.id}`}

...
export default function Main() {
  const [location_assigns, setLocationAssign] = useState<any[]>([]);
  ...
  const fetchLocationAssign = async (page = 1, search = "") => {
    try {
      setLoading(true);

      const params: any = { page, per_page: perPage };
      if (search) params.search_key = search;
      const res = await api.get<ListApiResponse<any[]>>(`/location-assign`, {
        params,
      });

      const response = res.data;

      setLocationAssign(response.data);
  ...
  const changeStatus = (id: number) => {
    confirmAlert(
      ...
      async () => {
        try {
          setStatusLoading(id);
          const res = await api.put(`/location-assign/change-status/${id}`);
          const message = (res.data as { message: string }).message;
          toast.success(message);
          fetchLocationAssign(currentPage);
        } catch (error: any) {
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
              <div className="flex items-center">
                ...
                <span className="text-gray-700 font-medium">Location Assign</span>
              </div>
            </li>
          </ol>
        </nav>
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center p-4 border-b border-gray-200 gap-3">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              fetchLocationAssign(1, e.target.value);
            }}
          />
        </div>
        ...
        <div className="p-4 overflow-x-auto">
          <table className="w-full table-auto text-sm text-left">
            ...
            <tbody>
              ...
              ) : location_assigns.length > 0 ? (
                location_assigns.map((val, index) => (
                  <tr
                    key={val.id}
                  ...
                  >
                    ...
                    <td className="px-4 py-3">{val.employee.name}</td>
                    <td className="px-4 py-3">{val.location.location_name}</td>
                    ...
                    <td className="px-4 py-3 text-center space-x-2">
                      <Link
                        to={`/master/location-assign/edit/${val.id}`}

...
interface Props {
  initialValues: LocationAssign;
  onSubmit: (
    values: LocationAssign,
    formikHelpers: FormikHelpers<LocationAssign>
  ) => void;
  mode: "create" | "edit";
}
...
export default function MasterForm({ initialValues, onSubmit, mode }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchStates = async () => {
      try {
        const res = await api.get("common/get-location-states");
  ...
  const fetchDistricts = async (stateKey: string) => {
    try {
      const res = await api.get(`common/get-location-districts/${stateKey}`);
  ...
  const fetchLocations = async (districtKey: string) => {
  try {
    const res = await api.get(`common/get-locations/${districtKey}`);

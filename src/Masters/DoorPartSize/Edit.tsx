...
export default function editFinishing() {
  ...
  const { id } = useParams();
  const [initialValues, setInitialValues] = useState<DoorPartSize | null>(null);


  useEffect(() => {
    const fetchFinishing = async () => {
      try {
        const res = await api.get(`/door-part-size/${id}/edit`);
  ...
  const handleSubmit = async (values: DoorPartSize,{ setErrors }: FormikHelpers<DoorPartSize>) => {
    try {
      const res =await api.put(`/door-part-size/${id}`, values);
